...
class sbmmultilayer:
    ...
    def make_graph(self, list_texts, list_titles, list_hyperlinks):
        ...
        for doc_id in range(D):
            ...
            for word, count in c.items():
                ...
                edgeCount[
                    e
                ] = count  # assign weighting to edge based on number of occurrences
    ...
    def fit(self):
        ...
        for _ in range(self.n_init):
            ...
            state_temp = gt.minimize_nested_blockmodel_dl(
                self.g, deg_corr=True, layers=True, state_args=state_args
            )
        ...
        # Omit trivial levels: l=L-1 (single group), l=L-2 (bipartite)
            ...
            self.groups = {
                level: self.get_groupStats(l=level) for level in range(n_levels - 2)
            }
            self.n_levels = len(self.groups)
    ...
    def get_N(self):
        ...
        return int(
            np.sum(
                [
                    self.g.ep.edgeCount[e]
                    for e in self.g.edges()
                    if self.g.ep["edgeType"][e] == 0
                ]
            )
        )
    ...
    def plot(self, filename=None, nedges=1000):
        ...
        """
        g = self.g
        self.state.draw(output=filename, subsample_edges=nedges, layout="bipartite")
