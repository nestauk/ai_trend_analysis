...
def check_clusterings(label_U, label_V):
    ...
    if label_U.shape != label_V.shape:
        raise ValueError(
            f"label_U and label_V must be same size. Current sizes are {label_U.shape[0]} and {label_V.shape[0]}"
        )
    # Pass all test cases!
...
def create_contingency_matrix(labels_U, label_V):
    ...
    contingency_matrix = sp.coo_matrix(
        (np.ones(clusters_U_idx.shape[0]), (clusters_U_idx, cluster_V_idx)),
...
def compute_mutual_info_score(contingency_matrix):
    ...
    if sp.issparse(contingency_matrix):
        ...
        raise ValueError(
            "Unsupported type for given contingency matrix: %s"
    ...
    outer = pi.take(NZ_row_idx).astype(np.int64, copy=False) * pj.take(
        NZ_col_idx
...
def compute_normalised_mutual_information(
    labels_U, label_V, average_method="arithmetic"
):
    """
    ...
    if (
        clusters_U.shape[0] == clusters_V.shape[0] == 1
        or clusters_U.shape[0] == clusters_V.shape[0] == 0
    ):
        return 1.0
