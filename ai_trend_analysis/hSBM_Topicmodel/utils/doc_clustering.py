...
cmap = matplotlib.colors.ListedColormap(np.vstack(plt.cm.Set3.colors[:6]))
...
class doc_clustering:
    def __init__(
        ...
        output_path,
        hyperlink_graph,
        seeds=[1213, 28, 520, 39, 47, 111111, 9, 180, 823, 324],
    ):
        """
        ...
        for i in range(len(self.seeds)):
            try:
                ...
                partition = self.hyperlink_graph.vp[
                    "partition"
                ] = self.hyperlink_graph.new_vp("int")
...
def _nmis(partitions):
    ...
    n = len(
        partitions
    )  # 10 * 10, depends on number of iterations to retrieve partitions
    mi_matrix = np.zeros((n, n))
    for i in range(n):
        for j in range(i + 1, n):
            mi_matrix[i, j] = compute_normalised_mutual_information(
...
def construct_nmi_matrix(partitions, true_partition):
    ...
    for i in range(1, num_models + 1):
        for j in range(i, num_models + 1):
            ...
            )  # retrieve list of partitions for model i-1 and model j-1
            # Store mean and std of NMIs.
            nmis_avg[i, j] = np.average(nmis)
            nmis_std[i, j] = np.std(nmis)

    nmis_avg[0, 0], nmis_std[0, 0] = (
        1,
        0,
    )  # true partition should have NMI of 1 with itself.
    ...
    for i in range(num_models):
        ...
        nmis_with_true = [
            compute_normalised_mutual_information(p, true_partition)
...
cmaps = [
    matplotlib.colors.ListedColormap(np.vstack(plt.cm.Set3.colors[:6])),
    matplotlib.colors.ListedColormap(np.vstack(plt.cm.Set3.colors[:6][::-1])),
]
...
def compute_nmi_plots(nmi_avg, nmi_std, methods):
    ...
    fig, ax = plt.subplots(
        1, 2, sharey=True, figsize=(12, 5), gridspec_kw=dict(hspace=0.1)
    )
